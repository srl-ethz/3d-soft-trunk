...
ControllerPCC::ControllerPCC(const SoftTrunkParameters st_params, CurvatureCalculator::SensorType sensor_type, int objects) : st_params(st_params), sensor_type(sensor_type), objects(objects){
    ...
    // +X, +Y, -X, -Y
    std::vector<int> map = {4,6,12,5,10,3,11};
...
void ControllerPCC::toggleGripper(){
    ...
    gripping = !gripping;
    vc->setSinglePressure(3*st_params.num_segments, gripping*270);
}
...
void ControllerPCC::toggle_log(){
    if(!logging) {
        ...
        //write header
        log_file << fmt::format(", x, y, z, x_ref, y_ref, z_ref, err, model_time, total_time");
...
void ControllerPCC::log(double time){
    ...
    */
    log_file << fmt::format(", {}, {}, {}, {}, {}, {}, {}, {}, {}", x(0), x(1), x(2), x_ref(0), x_ref(1), x_ref(2), (x - x_ref).norm(),model_time,tot_time);
