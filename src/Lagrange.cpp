...
void Lagrange::p_update(VectorXd q)
{
  ...
  p_FK_tmp = L[1] * t4;
  b_p_FK_tmp = L[1] * t8;
  c_p_FK_tmp = L[1] * t6 * t7;
  p_FK[0] = ((L[0] * t6 * t13 * (t3 - 1.0) - c_p_FK_tmp * t9 * t14) + b_p_FK_tmp
             * t14 * (t5 - 1.0) * (t11 + t3 * (t6 * t6))) + p_FK_tmp * t12 * t14
    * (t3 - 1.0) * (t5 - 1.0) / 2.0;
  t6 = L[1] * t2;
  p_FK[1] = ((L[0] * t2 * t13 * (t3 - 1.0) - t6 * t7 * t9 * t14) + p_FK_tmp *
             t14 * (t5 - 1.0) * ((-t11 + t3 * t11) + 1.0)) + b_p_FK_tmp * t12 *
...
void Lagrange::J_update(VectorXd q)
{
  ...
  b_Jac_tmp = L[1] * t4;
  c_Jac_tmp = L[0] * t2;
  d_Jac_tmp = b_Jac_tmp * t17;
  e_Jac_tmp = Jac_tmp * t6 * t8 * t17 * (t3 - 1.0) * (t5 - 1.0);
  f_Jac_tmp = Jac_tmp * t7 * t9;
  Jac[0] = ((c_Jac_tmp * t15 * (t3 - 1.0) - f_Jac_tmp * t17) + d_Jac_tmp * (t3 -
             1.0) * (t5 - 1.0) * t27) + e_Jac_tmp * 2.0;
  g_Jac_tmp = -L[0] * t6;
  Jac_tmp_tmp = L[1] * t6 * t7;
  h_Jac_tmp = Jac_tmp_tmp * t9;
  i_Jac_tmp = L[1] * t8;
  j_Jac_tmp = Jac_tmp * t4;
  Jac[1] = ((g_Jac_tmp * t15 * (t3 - 1.0) + h_Jac_tmp * t17) + i_Jac_tmp * t17 *
            (t3 - 1.0) * (t5 - 1.0) * t27) - j_Jac_tmp * t6 * t17 * (t3 - 1.0) *
    (t5 - 1.0) * 2.0;
  Jac[2] = t30;
  t27 = t30_tmp * t8;
  k_Jac_tmp = b_Jac_tmp * t7;
  b_Jac_tmp_tmp = L[1] * t3;
  l_Jac_tmp = b_Jac_tmp_tmp * t6;
  Jac[3] = (((g_Jac_tmp * t7 * t15 - L[0] * t6 * t16 * (t3 - 1.0)) - l_Jac_tmp *
             t9 * t17) - k_Jac_tmp * t12 * t17 * (t5 - 1.0) / 2.0) - t27 * t13 *
    t17 * (t5 - 1.0);
  Jac_tmp *= t3;
  Jac[4] = (((-L[0] * t2 * t7 * t15 - c_Jac_tmp * t16 * (t3 - 1.0)) - Jac_tmp *
             t9 * t17) - t27 * t12 * t17 * (t5 - 1.0) / 2.0) + k_Jac_tmp * t17 *
    (t5 - 1.0) * -t11;
  Jac[5] = (((-L[0] * t3 * t15 + L[0] * t7 * t16) + t30_tmp * t9 * t17) -
            Jac_tmp * t4 * t17 * (t5 - 1.0)) - l_Jac_tmp * t8 * t17 * (t5 - 1.0);
  Jac[6] = d_Jac_tmp * (t5 - 1.0) * t28 - e_Jac_tmp;
  Jac_tmp = b_Jac_tmp * t12;
  Jac[7] = -L[1] * t8 * t17 * (t5 - 1.0) * t29 + Jac_tmp * t17 * (t3 - 1.0) *
    (t5 - 1.0) / 2.0;
  Jac[8] = -t30;
  c_Jac_tmp = b_Jac_tmp * t9;
  d_Jac_tmp = i_Jac_tmp * t9;
  Jac[9] = ((((-L[1] * t5 * t6 * t7 * t17 + h_Jac_tmp * t18) - d_Jac_tmp * t17 *
              t28) - i_Jac_tmp * t18 * (t5 - 1.0) * t28) - c_Jac_tmp * t12 * t17
            * (t3 - 1.0) / 2.0) - Jac_tmp * t18 * (t3 - 1.0) * (t5 - 1.0) / 2.0;
  Jac[10] = ((((-L[1] * t2 * t5 * t7 * t17 + f_Jac_tmp * t18) - c_Jac_tmp * t17 *
               t29) - b_Jac_tmp * t18 * (t5 - 1.0) * t29) - d_Jac_tmp * t12 *
             t17 * (t3 - 1.0) / 2.0) - i_Jac_tmp * t12 * t18 * (t3 - 1.0) * (t5
    - 1.0) / 2.0;
  Jac_tmp = j_Jac_tmp * t7;
  b_Jac_tmp = Jac_tmp_tmp * t8;
  Jac[11] = ((((-L[1] * t3 * t5 * t17 + b_Jac_tmp_tmp * t9 * t18) + Jac_tmp * t9
               * t17) + b_Jac_tmp * t9 * t17) + Jac_tmp * t18 * (t5 - 1.0)) +
    b_Jac_tmp * t18 * (t5 - 1.0);
...
void Lagrange::JDot_update(VectorXd q, VectorXd dq)
{
  ...
  t96_tmp = L[0] * t2;
  b_t50_tmp = t50_tmp * t6;
  b_t96_tmp = t96_tmp * t7;
  t96 = (((b_t96_tmp * t19 + t50) + t96_tmp * t20 * (t3 - 1.0)) + b_t50_tmp * t7
  ...
  t47_tmp = t88_tmp * t22 * (t3 - 1.0) * (t5 - 1.0);
  t56 = L[1] * t22;
  JacDot_tmp = t10 * t22 * (t3 - 1.0) * (t5 - 1.0);
  b_JacDot_tmp = L[1] * t15;
  c_JacDot_tmp = t99_tmp_tmp * t9;
  d_JacDot_tmp = t2 * t45 + -(t2 * t47);
  JacDot[0] = ((-dq[1] * t96 - dq[0] * ((((-t60 + t97_tmp * t19 * (t3 - 1.0)) +
    t56 * (t3 - 1.0) * (t5 - 1.0) * t41) + b_JacDot_tmp * t16 * t22 * (t3 - 1.0)
    * (t5 - 1.0)) + JacDot_tmp * 4.0)) - dq[3] * ((((d_JacDot_tmp + c_JacDot_tmp
    * t22 * (t3 - 1.0) * (t28 - 1.0)) + t99_tmp_tmp * t23 * (t3 - 1.0) * (t5 -
    1.0) * (t28 - 1.0)) + b_t50_tmp * t9 * t15 * t22 * (t3 - 1.0)) + b_t50_tmp *
    t15 * t23 * (t3 - 1.0) * (t5 - 1.0))) + t47_tmp * t40;
  t56_tmp = t99_tmp_tmp * t22;
  b_t56_tmp = L[1] * t9;
  t50 = b_t56_tmp * t22 * (t3 - 1.0);
  b_t47_tmp = t6 * t45 + -(t6 * t47);
  t50_tmp_tmp = L[1] * t23 * (t3 - 1.0) * (t5 - 1.0);
  JacDot[1] = ((dq[1] * t29 + dq[3] * ((((b_t47_tmp + t50 * -t8 * (t28 - 1.0)) +
    t50_tmp_tmp * -t8 * (t28 - 1.0)) + t10 * t9 * t22 * (t3 - 1.0) * 2.0) + t10 *
    t23 * (t3 - 1.0) * (t5 - 1.0) * 2.0)) - dq[0] * ((((-t55 + t96_tmp * t19 *
    (t3 - 1.0)) - t99_tmp_tmp * t16 * t22 * (t3 - 1.0) * (t5 - 1.0) * 2.0) +
    t56_tmp * (t3 - 1.0) * (t5 - 1.0) * t28) + b_t50_tmp * t8 * t22 * (t3 - 1.0)
    * (t5 - 1.0) * 4.0)) + t47_tmp * t39;
  JacDot[2] = ((t86 + t87) - t88) - t93;
  t47_tmp = L[0] * t3;
  t30 = L[1] * t8;
  b_t50_tmp = t30 * t22 * (t5 - 1.0);
  JacDot[3] = ((dq[1] * (((((t60 - t47_tmp * t6 * t19) + b_t97_tmp * t20 * 2.0)
    + t97_tmp * t21 * (t3 - 1.0) * 2.0) - t56 * (t5 - 1.0) * t38 / 2.0) -
    b_t50_tmp * t34) - dq[0] * t96) + dq[3] * t100) - b_t88_tmp * (t4 - t39) /
    2.0;
  t56 = t56_tmp * (t5 - 1.0);
  t56_tmp = t8 - t40;
  JacDot[4] = ((dq[1] * (((((t55 - t96_tmp * t3 * t19) + b_t96_tmp * t20 * 2.0)
    + t96_tmp * t21 * (t3 - 1.0) * 2.0) - t56 * t32) - t56_tmp_tmp * t8 * t14 *
    t22 * (t5 - 1.0) / 2.0) + dq[0] * t29) + dq[3] * t99) + b_t88_tmp * t56_tmp /
    2.0;
  t29 = L[0] * t7;
  JacDot[5] = ((dq[3] * t98 + dq[1] * (((((t47_tmp * t20 * 2.0 + t29 * t19) -
    t29 * t21 * 2.0) + t56_tmp_tmp * t9 * t22) + t97_tmp_tmp * t7 * t22 * (t5 -
    1.0)) + t60_tmp * t8 * t22 * (t5 - 1.0))) + t86_tmp * t3 * t22 * (t5 - 1.0) *
               t31) - t88_tmp * t3 * t22 * (t5 - 1.0) * t31;
  t47_tmp = t93_tmp * t23 * t42;
  t29 = t86_tmp * t22 * (t3 - 1.0) * (t5 - 1.0);
  t10 = t4 * t34;
  JacDot[6] = ((-dq[2] * (b_t50_tmp * t54 + JacDot_tmp) - t47_tmp * (((t4 * t13
    + t10) + t2 * t6 * t8) + t2 * t3 * t6 * -t8)) + t29 * t40) - t87_tmp * t6 *
    t7 * t22 * (t5 - 1.0) * t31;
  JacDot_tmp = t30 * t14;
  JacDot[7] = ((-dq[2] * (t56 * t76 + JacDot_tmp * t22 * (t3 - 1.0) * (t5 - 1.0)
    / 2.0) + t47_tmp * t95 / 2.0) + t87_tmp * t7 * t22 * (t5 - 1.0) * t56_tmp /
               2.0) + t29 * t39;
  JacDot[8] = ((-t86 - t87) + t88) + t93;
  t47_tmp = t45_tmp * t22;
  t56 = t30 * t9 * t14;
  t56_tmp = t88_tmp * t23 * t42;
  JacDot[9] = ((dq[1] * t100 + dq[3] * ((((((((t60 + t45_tmp * t6 * t7 * t23 *
    2.0) - b_t60_tmp * t24 * 2.0) - t47_tmp * (t3 - 1.0) * t33 / 2.0) +
    b_t56_tmp * t23 * (t3 - 1.0) * t33) + b_t56_tmp * t15 * t23 * t54) + t47_tmp
    * -t8 * t54) + L[1] * t24 * (t3 - 1.0) * (t5 - 1.0) * t33) + b_JacDot_tmp *
    t24 * (t5 - 1.0) * t54)) - dq[0] * ((((d_JacDot_tmp + c_JacDot_tmp * t12 *
    t22 * (t3 - 1.0)) + t56 * t22 * (t3 - 1.0)) + t99_tmp_tmp * t12 * t23 * (t3
    - 1.0) * (t5 - 1.0)) + JacDot_tmp * t23 * (t3 - 1.0) * (t5 - 1.0))) -
    t56_tmp * (((t8 * t14 + t4 * t28) + t10 * 2.0) + t3 * t14 * -t8) / 2.0;
  JacDot[10] = ((dq[1] * t99 + dq[3] * ((((((((t55 + t50_tmp * t5 * t7 * t23 *
    2.0) - t55_tmp * t24 * 2.0) - t99_tmp_tmp * t5 * t22 * t76) + c_JacDot_tmp *
    t23 * t76 * 2.0) + t99_tmp_tmp * t24 * (t5 - 1.0) * t76 * 2.0) - t45_tmp *
    t8 * t14 * t22 * (t3 - 1.0) / 2.0) + t56 * t23 * (t3 - 1.0)) + JacDot_tmp *
    t24 * (t3 - 1.0) * (t5 - 1.0))) + dq[0] * ((((b_t47_tmp + t50 * t33) +
    t50_tmp_tmp * t33) + b_t56_tmp * t12 * t22 * (t3 - 1.0) * -t8) + L[1] * t12 *
    t23 * (t3 - 1.0) * (t5 - 1.0) * -t8)) + t56_tmp * t95 / 2.0;
  t47_tmp = t3 * t9;
  t56 = t2 * t4;
  JacDot_tmp = t56 * t7;
  b_JacDot_tmp = t6 * t7;
  t56 = t56 * t5 * t7;
  c_JacDot_tmp = t5 * t6 * t7 * t8;
  JacDot[11] = ((dq[1] * t98 + t93_tmp * t24 * ((((((((((t47_tmp * -2.0 + q[3] *
    t3 * t5 * 2.0) + JacDot_tmp * 2.0) + t47_tmp * t11) + b_JacDot_tmp * t15) -
    t56 * 2.0) - c_JacDot_tmp * 2.0) - JacDot_tmp * t17 * 2.0) - b_JacDot_tmp *
    t8 * t17 * 2.0) + t56 * t11) + c_JacDot_tmp * t11)) - b_t86_tmp * t23 * t31 *
                t42) + t88_tmp_tmp * t23 * t31 * t42;
